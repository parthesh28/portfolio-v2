      ...
      <div className="w-full max-w-2xl flex flex-col h-auto bits-container-adaptive md:max-h-[75vh]">
          ...
          <nav aria-label="content filter" className="brutalist flex p-0.5 sm:p-1 gap-1 shrink-0">
            {FILTERS.map((type) => (
              <button
                ...
                className={`px-3 py-1 text-xs sm:px-4 sm:py-1 sm:text-sm font-bold cursor-pointer font-pixel-square ${filter === type
        ...
        <section aria-label="bits list" className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden p-1.5 sm:p-3 flex flex-col gap-2.5 sm:gap-3">
          {displayedBits.length > 0 ? (
            ...
            <div className="py-6 sm:py-10 flex items-center justify-center opacity-50 font-pixel-square text-xs sm:text-sm">
              [ nothing here ]
            </div>
          )}
        </section>
        ...
        <footer className="mt-3 sm:mt-4 flex justify-center items-center gap-3 sm:gap-4 shrink-0 pt-1 sm:pt-2">
          <button
            ...
            className="brutalist cursor-pointer w-8 h-8 md:w-10 md:h-10 flex items-center justify-center disabled:invisible text-xs sm:text-base font-bold font-pixel-square"
          >
